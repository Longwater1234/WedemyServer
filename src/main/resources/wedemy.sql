...
INSERT INTO `lessons` (`id`, `lesson_name`, `videokey`, `course_id`, `video_length`)
VALUES (UUID_TO_BIN(UUID()), '1. Introduction to Java', '2dZiMBwX_5Q', 10012, 455),
       (UUID_TO_BIN(UUID()), '2. Installation and Hello World', 'Hdf5OmERt0g', 10012, 323),
       (UUID_TO_BIN(UUID()), '3. Understanding Java Foundation', 'ovb8njlzvlA', 10012, 688),
       (UUID_TO_BIN(UUID()), '4. Arguments and Parameters', 'U5b9TH14REM', 10012, 690),
       (UUID_TO_BIN(UUID()), '5. Input and Output', 'JceW6zvmA_Q', 10012, 564),
       (UUID_TO_BIN(UUID()), '6. Variables', '7wWvSn_qiBc', 10012, 531),
       (UUID_TO_BIN(UUID()), '7. Primitives and Objects', 'r4wWYgkBcpI', 10012, 507),
       (UUID_TO_BIN(UUID()), '8. Variable Declaration and Initialization', '1mRN2MwdWUo', 10012, 478),
       (UUID_TO_BIN(UUID()), '9. Primitive Data Types', 'qUXbJziVs_o', 10012, 660),
       (UUID_TO_BIN(UUID()), 'DAY 1 - Lose Weight and Lose Belly Fat', 'By6GXzcldGY', 10016, 1320),
       (UUID_TO_BIN(UUID()), 'DAY 2 - Lose Weight and Lose Belly Fat', '4920WOvqqPQ', 10016, 1311),
       (UUID_TO_BIN(UUID()), 'DAY 3 - Lose Weight and Lose Belly Fat', 'rqFYiJNOZjY', 10016, 1338),
       (UUID_TO_BIN(UUID()), 'DAY 4 - Lose Weight and Lose Belly Fat', 'MnUflUjr2bw', 10016, 1329),
       (UUID_TO_BIN(UUID()), 'DAY 5 - Lose Weight and Lose Belly Fat', 'lSZacNnqn7g', 10016, 1327),
       (UUID_TO_BIN(UUID()), 'DAY 6 - Lose Weight and Lose Belly Fat', '2tHo7phAtfM', 10016, 1317),
       (UUID_TO_BIN(UUID()), 'DAY 7 - Lose Weight and Lose Belly Fat', '0nhfE3q6ZA8', 10016, 1313),
       (UUID_TO_BIN(UUID()), 'DAY 8 - Lose Weight and Lose Belly Fat', 'ex16_HWvYJM', 10016, 1315),
       (UUID_TO_BIN(UUID()), 'DAY 9 - Lose Weight and Lose Belly Fat', 'mbNl_XaWjWQ', 10016, 1312),
       (UUID_TO_BIN(UUID()), 'DAY 10 - Lose Weight and Lose Belly Fat', 'x9MJjko7vJM', 10016, 1318),
       (UUID_TO_BIN(UUID()), '1. How to Get Started With JavaScript', 'ItYye9h_RXg', 10011, 208),
       (UUID_TO_BIN(UUID()), '2. Which Tools to Use When Developing', 'ns_L4kpxY8c', 10011, 268),
       (UUID_TO_BIN(UUID()), '3. How to Install Extensions For JavaScript Development', 'ZouJQRxgem0',
        10011, 381),
       (UUID_TO_BIN(UUID()), '4. How to Include JavaScript in Our HTML', 'AD5hxsFJc4o', 10011, 499),
       (UUID_TO_BIN(UUID()), '5. How to Use the Developer Tool for JavaScript Development',
        'sjmyfwESv1g', 10011, 382),
       (UUID_TO_BIN(UUID()), '6. Rules for Writing JavaScript Code', 'FdlBtidhAnE', 10011, 338),
       (UUID_TO_BIN(UUID()), '7. How to Create Variables in JavaScript', '9aGIAL16DL4', 10011, 537),
       (UUID_TO_BIN(UUID()), '8. Different Data Types in JavaScript', 'O9by2KcR2v4', 10011, 880),
       ...
       (UUID_TO_BIN(UUID()), '1. Preferences and Project Settings', 'EFK-vhzvYDg', 10014, 1137),
       (UUID_TO_BIN(UUID()), '2. Timeline Scroll: Page VS Smooth', 'GRESV6TO5sA', 10014, 162),
       (UUID_TO_BIN(UUID()), '3. Scale or Set to Frame Size', 'kh5wWqyG1KU', 10014, 562),
       (UUID_TO_BIN(UUID()), '4. Keyboard Shortcuts', 'aJTn5pj2zZ0', 10014, 409),
       (UUID_TO_BIN(UUID()), '5. How to Setup a New Project', 'Ud3ABhhJwb0', 10014, 766),
       (UUID_TO_BIN(UUID()), '6. Windows and Layouts', 'J-XuNBVG_lg', 10014, 396),
       (UUID_TO_BIN(UUID()), '7. Importing Media Part 1', 't7hGBgkOJDY', 10014, 607),
       (UUID_TO_BIN(UUID()), '8. Importing Media Part 2', 'Y_aydeNr8l4', 10014, 257),
       (UUID_TO_BIN(UUID()), '9. The Project Panel', 'UTtrFgapV_4', 10014, 653),
       (UUID_TO_BIN(UUID()), '10. The Source Panel', 'J7xy5l6IsMk', 10014, 432),
       (UUID_TO_BIN(UUID()), '1. Excel Basics Tutorial', 'rwbho0CgEAE', 10019, 1307),
       (UUID_TO_BIN(UUID()), '2. Intermediate Excel Skills, Tips, and Tricks', 'lxq_46nY43g', 10019,
        1218),
       (UUID_TO_BIN(UUID()), '3. VLOOKUP Basics', 'y8ygx1Zkcgs', 10019, 718),
       (UUID_TO_BIN(UUID()), '4. Creating Pivot Tables in Excel', 'BkmxrvIfDGA', 10019, 791),
       (UUID_TO_BIN(UUID()), '5. Using Recommended Pivot Tables', 'ebdgGbsTWs8', 10019, 317),
       (UUID_TO_BIN(UUID()), '6. Protecting a Sheet', 'piIWTp3qncw', 10019, 255),
       (UUID_TO_BIN(UUID()), '7. Advanced Excel - 3D Formulas', 'arJBUarj8u8', 10019, 443),
       (UUID_TO_BIN(UUID()), '8. Data Validation and Drop-Down Lists', 'SlWIgMFpsPg', 10019, 788),
       (UUID_TO_BIN(UUID()), '9. Creating Your Own Excel Templates', 'dgHjAHIBvsI', 10019, 345),
       (UUID_TO_BIN(UUID()), '10. Excel Split Names', 'yCxnWvD_r_Q', 10019, 314),
       (UUID_TO_BIN(UUID()), '1. The Piano Keyboard', 'QBH6IpRkVDs', 10018, 698),
       (UUID_TO_BIN(UUID()), '2. Starting to Read Music', '3BULT0-joT0', 10018, 1182),
       (UUID_TO_BIN(UUID()), '3. Reading a Melody', 'NUVQIwO1SEI', 10018, 984),
       (UUID_TO_BIN(UUID()), '4. The Left Hand And The Scale Of C Major', 'f9JI_5y0K68', 10018, 987),
       (UUID_TO_BIN(UUID()), '5. Learning a Piece', '1JVtPB8VJXE', 10018, 881),
       (UUID_TO_BIN(UUID()), '6. A New Piece, A New Scale, And Rests', 'yeP2qRcHuUM', 10018, 796),
       (UUID_TO_BIN(UUID()), '7. Quavers (Eighth Notes) And Accidentals', '3UetN01yPTs', 10018, 864),
       (UUID_TO_BIN(UUID()), '8. A New Piece, Phrase Marks And Ritenuto', '7x20caWKKkQ', 10018, 749),
       (UUID_TO_BIN(UUID()), '9. The Concept Of Musical Key', 'aU3VEy-4qwM', 10018, 788),
       (UUID_TO_BIN(UUID()), '10. More Scales, And Playing Staccato', 'ZCdfa9GyyuM', 10018, 1114),
       ...
       (UUID_TO_BIN(UUID()), '4. Economic Systems and Macroeconomics: Crash Course Economics',
        'B43YEW2FvDs', 10015, 617),
       (UUID_TO_BIN(UUID()), '5. Supply and Demand: Crash Course Economics', 'g9aDizJpd_s', 10015, 622),
       (UUID_TO_BIN(UUID()), '6. Macroeconomics: Crash Course Economics', 'd8uTB5XorBw', 10015, 823),
       ...
       (UUID_TO_BIN(UUID()), '1. Install and Setup for Mac and Windows', 'YYXdXT2l-Gg', 10010, 928),
       (UUID_TO_BIN(UUID()), '2. Strings - Working with Textual Data', 'k9TUPpGqYTo', 10010, 1271),
       (UUID_TO_BIN(UUID()), '3. Integers and Floats - Working with Numeric Data', 'khKv-8q7YmY', 10010,
        715),
       (UUID_TO_BIN(UUID()), '4. Lists, Tuples, and Sets', 'W8KRzm-HUcc', 10010, 1744),
       ...
       (UUID_TO_BIN(UUID()), '6. Conditionals and Booleans - If, Else, and Elif Statements',
        'DZwmZ8Usvnk', 10010, 988),
       (UUID_TO_BIN(UUID()), '7. Loops and Iterations - For/While Loops', '6iF8Xb7Z3wQ', 10010, 614),
       (UUID_TO_BIN(UUID()), '8. Functions', '9Os0o3wzS_I', 10010, 1307),
       ...
       (UUID_TO_BIN(UUID()), '3. Artboards in Adobe Illustrator', '9GbLm_WXWwk', 10013, 579),
       (UUID_TO_BIN(UUID()), '4. Vector basics | Selection & Direct selection tool & more',
        'GFY0_EMVYDw', 10013, 1033),
       ...
       (UUID_TO_BIN(UUID()), '9. Grouping | Compounding vectors & Using the shape builder tool',
        'aGFWmYHUQOU', 10013, 901),
       (UUID_TO_BIN(UUID()), '10. The Blob brush tool & Eraser tool in Adobe Illustrator', 'wnXDboK7FH8',
        10013, 483),
       (UUID_TO_BIN(UUID()), 'Part 1 - Real Estate Investing', 'nFH8PV_jPLk', 10017, 344),
       (UUID_TO_BIN(UUID()), 'Part 2 - Real Estate Investing', 'dgu5PaLFu5Y', 10017, 452),
       (UUID_TO_BIN(UUID()), 'Part 3 - Real Estate Investing', '4c6afHE7P6M', 10017, 261),
       (UUID_TO_BIN(UUID()), 'Part 4 - Real Estate Investing', 'mRzoImyFMSY', 10017, 1880),
       (UUID_TO_BIN(UUID()), 'Part 5 - Real Estate Investing', '1mecyBhnJKg', 10017, 896);
COMMIT;
